...
class OpenSeaNftProvider(
    ...
    private fun assetMetadata(
        ...
        val bidOrders = response.orders.filter { it.side == "bid" && it.orderType == "criteria" }
        val offers = bidOrders.mapNotNull { order ->
            order.offer.firstOrNull()?.let { offer ->
                map[offer.token.lowercase()]?.let { token ->
                    nftPrice(token, order.currentPrice, true)
                }
            }
        }

        val basicAskOrders = response.orders.filter { it.side == "ask" && it.orderType == "basic" }
        val englishAskOrders = response.orders.filter { it.side == "ask" && it.orderType == "english" }
        ...
        listings = orders.mapNotNull { order ->
            order.consideration.firstOrNull()?.let { consideration ->
                map[consideration.token.lowercase()]?.let { token ->
                    val price = NftPrice(token, order.currentPrice.movePointLeft(token.decimals))
